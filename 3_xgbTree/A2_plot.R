...
#model.dir = out.dir = paste0(wk.dir, "/3_xgbTree/pHt/model_2/retrain")
#model.dir = out.dir = paste0(wk.dir, "/3_xgbTree/pHt/model_2-noLength/retrain")
model.dir = out.dir = paste0(wk.dir, "/3_xgbTree/Tm/model_2")
### OTHER SETTINGS #############################################################
...
if(obsVSpredPlot){
  ...
  ggplot(data=PRED, aes(x=pred, y=obs)) +
    ...
    scale_colour_manual(values=c("#C77CFF", "#430d6b")) +
    labs(title=out.name, 
         y=expression(bold("Predicted")), 
         x=expression(bold("Observed")), 
         colour=NULL
    ) +
    annotate(geom="text", size=4, parse=TRUE, x=p.param[[target]][[2]], y=p.param[[target]][[3]], 
             label=lmEqn_string(x="obs", y="pred", data=PRED[PRED$dataType==dataType.met,
                                                             c("obs", "pred")])
    ) + 
    annotate(geom="text", size=6, label="xgbTree", fontface=2,
             x=p.param[[target]][[4]], y=p.param[[target]][[5]]
    ) + 
    bgr1 +
